...
function displaceChannel(from, to, roughness, depth, rand) {
  ...
  const midY = (from.y + to.y) / 2 + (rand() * 2 - 1) * roughness * (0.04 + rand() * 0.14);
...
function roughenPoints(points, width, rand, amount) {
  return points.map((p, i) => {
    if (i === 0 || i === points.length - 1 || rand() > 0.58) return { ...p };
    return {
...
function convergeToPoint(points, endX, endY, fromRatio) {
...
function makePath(points, kind, intensity, delay) {
  return { d: toPath(points), kind, intensity, delay };
}

export function generateBolt(width, height, seed, options = {}) {
  const rand = makeRand(seed);
  const endX = options.endX ?? width / 2;
  const endY = options.endY ?? height;
  const startX = options.startX ?? endX + (rand() * 2 - 1) * width * (0.12 + rand() * 0.14);
  ...
  for (let b = 0; b < branchCount; b += 1) {
    ...
    forks.push(makePath(branchPts, "branch", 0.45 + rand() * 0.55, rand() * 0.12));
    ...
    for (let t = 0; t < twigCount; t += 1) {
      ...
      forks.push(makePath(twigPts, "twig", 0.25 + rand() * 0.45, rand() * 0.16));
    }
  }

  if (rand() > 0.2) {
    const spurEligible = mainPts.filter((p) => p.y > endY * 0.06 && p.y < branchOriginMaxY * 0.85);
  ...
  for (let h = 0; h < hairCount && hairEligible.length > 0; h += 1) {
    ...
    forks.push(makePath([origin, hairEnd], "twig", 0.12 + rand() * 0.22, rand() * 0.14));
  }
